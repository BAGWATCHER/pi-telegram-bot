...
app.post("/message", async (req: Request, res: Response) => {
  ...
  try {
    ...
    let finalText = fullText.trim() || session.getLastAssistantText()?.trim() || "";

    if (!finalText) {
      // Retry once on empty output glitches.
      await session.prompt(text, { streamingBehavior: 'followUp' });
      finalText = fullText.trim() || session.getLastAssistantText()?.trim() || "";
    }
    ...
    if (!finalText) {
      finalText = "⚠️ Empty response glitch. Please resend your last message.";
    }
